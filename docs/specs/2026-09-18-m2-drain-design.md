...
| `max_leaf_clusters` | 128 | *(Added 2026-09-22.)* Bounds one line's work: search scores every cluster in the line's leaf, so without it a source sharing one prefix makes each line scan up to `max_clusters` clusters (about 2 ms at 4,096 clusters and 128 tokens, and growing). Past the cap the leaf's least recently used cluster is evicted. 128 sits above the largest real leaf in the loghub-2.0 samples (Spark, 75), costs no GA there at any cap from 32 up, and holds a hostile stream at 14 µs a line at 16 tokens (`benches/worst_case.rs`). Proved: `leaf_cost_bounded`. |
